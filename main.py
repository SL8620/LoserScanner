...
class PyQtMainEntry(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.img = None  # 原图像
        self.resizedImg = None  # 当图像大于电脑屏幕时需要缩放（手机像素太高）
        self.boxedImg = None  # 显示裁剪范围的图像
        self.resultImg = None  # 处理结果
        self.scale = 1  # 缩放倍数，后期定点还原需要用到
    ...
    def pickPic_clicked(self):
        ...
        if filename:  # 读取到了图像
            ...
            self.resizedImg = self.img
            if self.picSize[0] > 800:
                self.scale = self.picSize[0] / 800
                print(self.scale)
                print([800, int(self.picSize[1] / self.scale)])
                self.resizedImg = cv.resize(self.img, None, fx=1 / self.scale, fy=1 / self.scale)
    ...
    def on_mouse(self, event, x, y, flags, param):
        ...
        if event == cv.EVENT_LBUTTONDOWN:  # 别看这种写法比较冗余，但这是手残党的福音，谁也没办法确定一次就能选好那个点
            if x < self.picSize[1] / self.scale / 2 and y < self.picSize[0] / self.scale / 2:  # 左上
                self.box[0] = (x, y)
                self.boxList[0] = [x, y]
            elif x > self.picSize[1] / self.scale / 2 and y < self.picSize[0] / self.scale/ 2:  # 右上
                self.box[1] = (x, y)
                self.boxList[1] = [x, y]
            elif x < self.picSize[1] / self.scale / 2 and y > self.picSize[0] / self.scale/ 2:  # 左下
                self.box[2] = (x, y)
                self.boxList[2] = [x, y]
            elif x > self.picSize[1] / self.scale / 2 and y > self.picSize[0] / self.scale/ 2:  # 右下
                self.box[3] = (x, y)
                self.boxList[3] = [x, y]

            # print(self.box)

            img_circle = self.resizedImg.copy()
            # 画圆
            for pos in self.box:
                if pos != 0:
            ...
            bytesPerLine = channels * cols
            QImg = QImage(self.boxedImg.data, cols, rows, bytesPerLine, QImage.Format_RGB888)
            self.boxedPic.setPixmap( QPixmap.fromImage(QImg).scaled(self.boxedPic.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation))
    ...
    def pickBox_clicked(self):  # 点了选择边框之后会发生的事
        if self.picPath:
            ...
            cv.imshow(self.mouseWindowName, self.resizedImg)  # 在这上面首次显示初始图片，会被后续的覆盖掉
            cv.waitKey(0)  # 点x才能退出

    def resultPic_clicked(self):  # 点了处理图片后会发生的事
        # print(self.boxList)
        for point in self.boxList:
            point[0] = int(point[0] * self.scale)
            point[1] = int(point[1] * self.scale)
        # print(self.boxList)
        testIMG = self.img.copy()
        for point in self.boxList:
            cv.circle(testIMG, point, 10, (0, 255, 0), thickness=3)
            #cv.imshow("scale",testIMG)
        cv.imwrite('test.png',testIMG)
        imgbak = np.array(self.img)  # 先转化成numpy里的array
        destPoint = [(0, 0), (self.picSize[1],0), (0, self.picSize[0]), (self.picSize[1], self.picSize[0])]  # 新图片目标点，大小和原图片是一样的
        pointList = [self.boxList[0], self.boxList[1], self.boxList[2], self.boxList[3]]  # 原图片目标点，要与楼上对应，这里选的是左上左下右下三点
        # (destPoint)
        # print(pointList)
        pts1 = np.float32(pointList)  # 听说转成float更好处理？
        pts2 = np.float32(destPoint)
        M = cv.getPerspectiveTransform(pts1, pts2)  # 本项目最核心的一句话，计算仿射变换矩阵
        dst = cv.warpPerspective(imgbak, M, (self.picSize[1], self.picSize[0]))  # 将仿射变换矩阵应用到原图像，生成目标图像
